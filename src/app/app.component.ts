...
export class AppComponent implements OnInit {
  ...
  createNav(event: { screenX: number; screenY: number; }) {
    
    if (this.mousePosition.x === event.screenX && this.mousePosition.y === event.screenY){//so the click function wont trigger when dragged
  ...
  createRadio(event: { screenX: number; screenY: number; }) {
    if (this.mousePosition.x === event.screenX && this.mousePosition.y === event.screenY){//so the click function wont trigger when dragged
  ...
  createDate(event: { screenX: number; screenY: number; }) {
    if (this.mousePosition.x === event.screenX && this.mousePosition.y === event.screenY){//so the click function wont trigger when dragged
  ...
  addImage(event: { screenX: number; screenY: number; }) {
    if (this.mousePosition.x === event.screenX && this.mousePosition.y === event.screenY){//so the click function wont trigger when dragged
  ...
  createDropdown(event: { screenX: number; screenY: number; }) {
    if (this.mousePosition.x === event.screenX && this.mousePosition.y === event.screenY){
      //so the click function wont trigger when dragged
  ...
  createModal(event: { screenX: number; screenY: number; }) {
    if (this.mousePosition.x === event.screenX && this.mousePosition.y === event.screenY){
      //so the click function wont trigger when dragged
  ...
  createCheckbox(event: { screenX: number; screenY: number; }) {
    if (this.mousePosition.x === event.screenX && this.mousePosition.y === event.screenY){
      //so the click function wont trigger when dragged
      const mainDiv = this.renderer.createElement('div');
  ...
  createLabel(event: { screenX: number; screenY: number; }) {
    if (this.mousePosition.x === event.screenX && this.mousePosition.y === event.screenY){
      //so the click function wont trigger when dragged
      const checkboxLabel = this.renderer.createElement('label');
  ...
  createInputField(event: { screenX: number; screenY: number; }) {
    if (this.mousePosition.x === event.screenX && this.mousePosition.y === event.screenY){
      //so the click function wont trigger when dragged
  ...
  createHeader(event: { screenX: number; screenY: number; }) {
    if (this.mousePosition.x === event.screenX && this.mousePosition.y === event.screenY){
      //so the click function wont trigger when dragged
  ...
  createParagraph(event: { screenX: number; screenY: number; }) {
    if (this.mousePosition.x === event.screenX && this.mousePosition.y === event.screenY){
      //so the click function wont trigger when dragged
      const container = this.renderer.createElement('div');
  ...
  createLink(event: { screenX: number; screenY: number; }) {
    if (this.mousePosition.x === event.screenX && this.mousePosition.y === event.screenY){
      //so the click function wont trigger when dragged
...
onDragEndedNav(event: CdkDragEnd)
{
  event.source._dragRef.reset();
  const { offsetLeft, offsetTop } = event.source.element.nativeElement;
  const { x, y } = event.distance;
  ...
  if(this.domInsideCanvas == true)
  {
    ...
    this.renderer.setStyle(newNav, 'position', 'absolute');
    this.renderer.setStyle(newNav, 'left', this.mousePosition2.x+ "px");
    this.renderer.setStyle(newNav, 'top', this.mousePosition2.y + "px");

    this.renderer.appendChild(this.canvas.nativeElement, newNav); //append the button to the canvas div
  }
}

onDragEndedRadio(event: CdkDragEnd)
{
  event.source._dragRef.reset();
  const { offsetLeft, offsetTop } = event.source.element.nativeElement;
  const { x, y } = event.distance;
  ...
  if(this.domInsideCanvas == true)
  {
    const mainDiv = this.renderer.createElement('div');
      ...
      this.renderer.setStyle(newLabel, 'position', 'absolute');
      this.renderer.setStyle(newLabel, 'left', this.mousePosition2.x+ "px");
      this.renderer.setStyle(newLabel, 'top', this.mousePosition2.y + "px");

      this.renderer.appendChild(this.canvas.nativeElement, mainDiv);
    }
}

onDragEndedDate(event: CdkDragEnd)
{
  event.source._dragRef.reset();
  const { offsetLeft, offsetTop } = event.source.element.nativeElement;
  const { x, y } = event.distance;
  ...
  if(this.domInsideCanvas == true)
  {
      ...
      this.renderer.setStyle(newDate, 'position', 'absolute');
      this.renderer.setStyle(newDate, 'left', this.mousePosition2.x+ "px");
      this.renderer.setStyle(newDate, 'top', this.mousePosition2.y + "px");

      this.renderer.appendChild(this.canvas.nativeElement, newDate);
    }
}

onDragEndedImage(event: CdkDragEnd)
{
  event.source._dragRef.reset();
  const { offsetLeft, offsetTop } = event.source.element.nativeElement;
  const { x, y } = event.distance;
  ...
  if(this.domInsideCanvas == true)
  {
      ...
      this.renderer.setStyle(newImage, 'left', this.mousePosition2.x+ "px");
      this.renderer.setStyle(newImage, 'top', this.mousePosition2.y + "px");
...
onDragEndedDrop(event: CdkDragEnd)
{
  event.source._dragRef.reset();
  const { offsetLeft, offsetTop } = event.source.element.nativeElement;
  const { x, y } = event.distance;
  ...
  if(this.domInsideCanvas == true)
  {
    ...
    this.renderer.setStyle(newDiv, 'left', this.mousePosition2.x+ "px");
    this.renderer.setStyle(newDiv, 'top', this.mousePosition2.y + "px");

    this.renderer.appendChild(this.canvas.nativeElement, newDiv);
    }
}

onDragEndedModal(event: CdkDragEnd)
{
  event.source._dragRef.reset();
  const { offsetLeft, offsetTop } = event.source.element.nativeElement;
  const { x, y } = event.distance;
  ...
  if(this.domInsideCanvas == true)
  {
    const modalContainer = this.renderer.createElement('div'); //create dom element
     ...
     this.renderer.setProperty(modalHeaderButton, 'type', 'button'); //add type attribute to button
      ...
      this.renderer.setStyle(modalContainer, 'position', 'absolute');
      this.renderer.setStyle(modalContainer, 'left', this.mousePosition2.x+ "px");
      this.renderer.setStyle(modalContainer, 'top', this.mousePosition2.y + "px");
...
onDragEndedCheck(event: CdkDragEnd)
{
  event.source._dragRef.reset();
  const { offsetLeft, offsetTop } = event.source.element.nativeElement;
  const { x, y } = event.distance;
  ...
  if(this.domInsideCanvas == true)
  {
    ...
    this.renderer.setStyle(mainDiv, 'position', 'absolute');
    this.renderer.setStyle(mainDiv, 'left', this.mousePosition2.x+ "px");
    this.renderer.setStyle(mainDiv, 'top', this.mousePosition2.y + "px");
    
    this.renderer.appendChild(this.canvas.nativeElement, mainDiv);  
    }
}

onDragEndedLabel(event: CdkDragEnd)
{
  event.source._dragRef.reset();
  const { offsetLeft, offsetTop } = event.source.element.nativeElement;
  const { x, y } = event.distance;
  ...
  if(this.domInsideCanvas == true)
  {
    ...
    this.renderer.setStyle(checkboxLabel, 'position', 'absolute');
    this.renderer.setStyle(checkboxLabel, 'left', this.mousePosition2.x+ "px");
    this.renderer.setStyle(checkboxLabel, 'top', this.mousePosition2.y + "px");

    this.renderer.appendChild(this.canvas.nativeElement, checkboxLabel);  
    }
}

onDragEndedInput(event: CdkDragEnd)
{
  event.source._dragRef.reset();
  const { offsetLeft, offsetTop } = event.source.element.nativeElement;
  const { x, y } = event.distance;
  ...
  if(this.domInsideCanvas == true)
  {
    ...
    this.renderer.setStyle(inputField, 'position', 'absolute');
    this.renderer.setStyle(inputField, 'left', this.mousePosition2.x+ "px");
    this.renderer.setStyle(inputField, 'top', this.mousePosition2.y + "px");

    this.renderer.appendChild(this.canvas.nativeElement, inputField);
  }
}

onDragEndedHeader(event: CdkDragEnd)
{
  event.source._dragRef.reset();
  const { offsetLeft, offsetTop } = event.source.element.nativeElement;
  const { x, y } = event.distance;
  ...
  if(this.domInsideCanvas == true)
  {
    ...
    this.renderer.setStyle(headerFunc, 'position', 'absolute');
    this.renderer.setStyle(headerFunc, 'left', this.mousePosition2.x+ "px");
    this.renderer.setStyle(headerFunc, 'top', this.mousePosition2.y + "px");

    this.renderer.appendChild(this.canvas.nativeElement, headerFunc);
  }
}

onDragEndedParagraph(event: CdkDragEnd)
{
  event.source._dragRef.reset();
  const { offsetLeft, offsetTop } = event.source.element.nativeElement;
  const { x, y } = event.distance;
  ...
  if(this.domInsideCanvas == true)
  {
    ...
    this.renderer.setStyle(container, 'left', this.mousePosition2.x+ "px");
    this.renderer.setStyle(container, 'top', this.mousePosition2.y + "px");

    this.renderer.appendChild(this.canvas.nativeElement, container);
  }
}

onDragEndedLink(event: CdkDragEnd)
{
  event.source._dragRef.reset();
  const { offsetLeft, offsetTop } = event.source.element.nativeElement;
  const { x, y } = event.distance;
  ...
  if(this.domInsideCanvas == true)
  {
    ...
    this.renderer.setStyle(Link, 'position', 'absolute');
    this.renderer.setStyle(Link, 'left', this.mousePosition2.x+ "px");
    this.renderer.setStyle(Link, 'top', this.mousePosition2.y + "px");
